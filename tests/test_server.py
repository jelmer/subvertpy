...
class ServerBackendTests(TestCase):
    def test_open_repository_raises(self):
        ...
        self.assertRaises(NotImplementedError, backend.open_repository, "/test")
...
class ServerRepositoryBackendTests(TestCase):
    ...
    def test_get_latest_revnum_raises(self):
        self.assertRaises(NotImplementedError, self.backend.get_latest_revnum)

    def test_log_raises(self):
        self.assertRaises(
            NotImplementedError, self.backend.log, None, "/", 0, 1, True, True, 0
        )

    def test_update_raises(self):
        self.assertRaises(NotImplementedError, self.backend.update, None, 1, "/", True)

    def test_check_path_raises(self):
        self.assertRaises(NotImplementedError, self.backend.check_path, "/", 0)

    def test_stat_raises(self):
        self.assertRaises(NotImplementedError, self.backend.stat, "/", 0)

    def test_rev_proplist_raises(self):
        self.assertRaises(NotImplementedError, self.backend.rev_proplist, 0)

    def test_get_locations_raises(self):
        self.assertRaises(NotImplementedError, self.backend.get_locations, "/", 0, [0])
